...
func TestDocsColumnNeedsIngressAndIsOptIn(t *testing.T) {
	_, needs, err := resolveColumns([]string{"alias", "docs"})
	if err != nil {
		t.Fatal(err)
	}
	if needs&needsIngress == 0 {
		t.Error("docs hangs off the ingress origin, so it needs the ingress list")
	}
}
